import discord, threading, os, json, httpx, itertools, string, sys, time
...
class Console:
    ...
    @staticmethod
    def print_logo():
        os.system('cls && title github.com/its-vichy - DmReact' if os.name == 'nt' else 'clear')
        print(Fore.LIGHTWHITE_EX + """
    DmReact - github.com/its-vichy               
        """)
...
class ReactionListener(discord.Client):
    ...
    async def on_ready(self):
        Console.printf(f'[+] Listenning on {self.user}')
    
    async def on_raw_reaction_add(self, payload):
        user_id = payload.member.id
        
        if user_id not in data.blacklist and payload.message_id == __config__['message_id']: # and payload.message_id == __config__['message_id']:
            data.blacklist.append(user_id)
            time.sleep(__config__['wait_time'])
            self.send_dm(user_id)
            data.refresh_file(user_id)
            

if __name__ == '__main__':
    Console.print_logo()
    ReactionListener().run(__config__['listenner_token'], bot=False)